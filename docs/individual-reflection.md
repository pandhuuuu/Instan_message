...
#### 2. Tim (Team)
Kerja sama tim berjalan dengan sinergi yang sangat baik, saling percaya, dan disiplin tinggi di sepanjang ketiga milestone. Kami menerapkan alur kerja berulang (*iterative*) yang dipandu oleh pengujian berkelanjutan dan Git, dengan pertemuan sinkronisasi rutin untuk menyepakati kontrak protokol sebelum menghubungkan antarmuka ke backend. Standar koding dijaga sangat tinggi, menerapkan prinsip modularitas, sanitasi input, dan penjagaan invarian data yang ketat. Pembagian kerja dilakukan secara berimbang: sebagian fokus pada logika backend, pemodelan skema atomik MongoDB, dan orkestrasi event Socket.IO; sebagian lainnya membangun antarmuka web modern bergaya WhatsApp Web lengkap dengan dialog konfigurasi IP/Port dinamis; serta fokus pada penjaminan mutu melalui pembuatan 63 skrip uji otomatis, pengujian 10 skenario manual GUI (total 73 spesifikasi pengujian), dan dokumentasi teknis formal. Pembagian kerja yang terstruktur ini membuat proses integrasi berjalan mulus tanpa hambatan.

#### 3. Individu (Individual)
Secara pribadi, saya sangat puas dengan performa dan kontribusi teknis saya dalam proyek ini. Peran utama saya mencakup perancangan logika *presence* dan pelacakan status pesan *real-time*, implementasi mekanisme *soft-delete* per-user di basis data untuk menjaga privasi, penyusunan berkas uji otomatis 63 kasus uji di `run_tests.js`, serta kontainerisasi multi-layanan menggunakan Docker Compose. Mendalami mekanisme *event loop* Node.js untuk membuktikan ketiadaan *deadlock* (Coffman conditions) dan kebebasan dari *race conditions* memperluas pemahaman saya tentang konkurensi sistem secara signifikan. Melihat seluruh 63 kasus uji otomatis lulus sempurna dan menyaksikan dua jendela browser saling bertukar pesan dengan latensi di bawah 50 milidetik memberikan kepuasan rekayasa perangkat lunak yang luar biasa dan keyakinan penuh akan kualitas produk akhir kami.
